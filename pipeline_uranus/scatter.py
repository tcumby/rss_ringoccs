...
for dir in ["I", "E"]:
    ...
    f_spm, f_offset, f_recon_coarse = np.loadtxt(
        "../tables/URING_" + dir + "_FOF.CSV", delimiter=","
    ).T
    # spline the results
    ...
    f_offset_coarse = np.append(
        f_offset_coarse, np.polyval(p2, f_spm[(f_spm >= fbreak)])
    )
    ...
    for rind, ring_name in enumerate(pipe.rnames):
        ...
        ra_deg = 76.5969
        dec_deg = 15.1117
        nhat_p = spice.radrec(1.0, ra_deg * spice.rpd(), dec_deg * spice.rpd())
        f0 = 8420430456.1
        f_sky_hz_vals = f_recon_coarse  # np.zeros(len(f_spm)) + f0
        geo_inst = occgeo.Geometry(
            1986,
            doy,
            "DSS-43",
            f_sky_hz_vals,
