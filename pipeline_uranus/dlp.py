...
def correct_phase(spm, phase, dir, ring_name):
    ...
    for i in range(len(tst)):
        if incr[i] > 0:
            phase[(spm >= tst[i]) & (spm < ten[i]) & (phase < prng[i])] += (
                incr[i] * 2.0 * np.pi
            )
        else:
            phase[(spm >= tst[i]) & (spm < ten[i]) & (phase > prng[i])] += (
    ...
    fit[:k] = np.polyval(
        np.polyfit(spm[k : int(3 * k / 2)], fit[k : int(3 * k / 2)], 2), spm[:k]
    )
    fit[-k:] = np.polyval(
        np.polyfit(spm[int(-3 * k / 2) : -k], fit[int(-3 * k / 2) : -k], 2), spm[-k:]
    )
    ...
    dwrap = (diff + np.pi) % (2.0 * np.pi) - np.pi
    # smooth for plotting
    dsmooth = np.convolve(dwrap, np.ones(100) / 100.0, "same")
...
for dir in ["I", "E"]:
    ...
    for ring_name in pipe.rnames:
        ...
        if os.path.exists(geofile) and os.path.exists(calfile):
            ...
            IQ_c = I + 1j * Q
            # signal phase
            phase = -1.0 * np.arctan2(np.imag(IQ_c), np.real(IQ_c))
            ...
            B_rad = np.interp(spm, geo[0], geo[6]) * np.pi / 180.0
            ...
            B = np.interp(rho, geo[3][rsort], geo[6][rsort]) * np.pi / 180.0
            ...
            phi_rl = np.interp(rho, geo[3][rsort], geo[4][rsort]) * np.pi / 180.0
            phi_ora = np.interp(rho, geo[3][rsort], geo[5][rsort]) * np.pi / 180.0
            ...
            tt_inst = ctt.calc_tau_thresh(
                spm,
                IQ_c,
                np.interp(spm, geo[0], geo[3]),
                np.interp(spm, geo[0], geo[8]),
                np.interp(spm, geo[0], geo[6]) * np.pi / 180.0,
                params,
                pipe.rings_spm[dir][ring_name],
            ...
            for j in range(len(tau_thresh)):
                ...
                if rho[j] >= rho_min and rho[j] <= rho_max:
                    ...
                    out.write(
                        ",".join(str(round(val, 8)).rjust(18) for val in vals) + "\n"
                    )
            out.close()
        # in file name not recognized, say so
        else:
            print(
                "One of the required GEO and CAL files not found."
                + "\n"
