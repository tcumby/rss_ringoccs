...
from rss_ringoccs.tools.write_uranus_output_files import write_output_files
import os
...
for profile_dir in ["I", "E"]:
    # for each ring
    for ring_name in pipe.rnames:

        # reconstruct DLP filepath
        dlp_dir = os.path.join("..", "output", profile_dir, ring_name)

        dlp_file = f"VGR2_X43_{profile_dir}_URING_{ring_name}_DLP_{pipe.dlp_res}M_{pipe.dlp_date}_{pipe.dlp_sn}.TAB"

        dlp_file_path = os.path.join(dlp_dir, dlp_file)
        # rebuild DLP instance
        if pipe.verbose:
            print("Importing DLP file")
            print("\t", dlp_file_path)
        dlp_inst = dlp(dlp_file_path)

        # get radial range of ring profile
        if pipe.ring_widths[profile_dir][ring_name] > 25:
            rho_min = (
                pipe.rings_km[profile_dir][ring_name]
                - pipe.ring_widths[profile_dir][ring_name]
            )
            rho_max = (
                pipe.rings_km[profile_dir][ring_name]
                + pipe.ring_widths[profile_dir][ring_name]
            ...
            rho_min = pipe.rings_km[profile_dir][ring_name] - 25
            rho_max = pipe.rings_km[profile_dir][ring_name] + 25

        # do reconstruction
        tau_inst = DiffCorr.DiffractionCorrection(
            dlp_inst,
            pipe.res,
            rng=[rho_min, rho_max],
            res_factor=1,
            psitype=pipe.psitype,
            wtype=pipe.window,
            fwd=False,
            norm=False,
            bfac=True,
