...
fail_file = open(os.path.join(path, err_file), "w")
...
def filename(file, type):
    ...
    file = (
        "_".join(splt for splt in splts[:5])
        + "_"
        + type
        + "_"
        + "_".join(splt for splt in splts[7:])
    )
...
for n, dir in enumerate(occdirs):
    ...
    for _m, file in enumerate(os.listdir(dir)):
        ...
        if "DLP" in file and "TAB" in file:
            ...
            try:
                ...
                # build DLP instance
                t1 = time.time()
                dlp_inst = rss.tools.ExtractCSVData(
                    geo_file, cal_file, dlp_file, verbose=args.verbose
                )
                t2 = time.time()
                read_time += t2 - t1
