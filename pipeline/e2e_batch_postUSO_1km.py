...
write_file = True
### RSRReader
rsr_file_list = os.path.join(
    "..", "tables", "rsr_1kHz_files_after_USO_failure_withuplink.txt"
)
...
kernels = os.path.join(
    "..", "tables", "e2e_kernels.ker"
)  # Path to meta-kernel or list of paths to
...
profile_range = [65000.0, 150000.0]  # Lower and upper radial bounds to the
...
files = [
    os.path.join("..", "data", line.strip("\n"))
    for line in open(rsr_file_list).readlines()
]
...
for ind in range(nfiles):
    ...
    try:
        ...
        rsr_inst = rss.rsr_reader.RSRReader(
            rsr_file, verbose=verbose, decimate_16khz_to_1khz=decimate_16khz_to_1khz
        )
        ...
        if dlp_inst_ing is not None:
            ...
            rss.tools.plot_summary_doc_v2(geo_inst, cal_inst, dlp_inst_ing, tau_inst)
...
total_batch_time = (final_time - init_time) / 60.0 / 60.0
