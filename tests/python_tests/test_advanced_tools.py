...
class TestAdvancedTools:
    def test_CompareTAU(
        self, test_root_dir: Path, test_data_dir: Path, figures_dir: Path
    ):

        # Location of the data for Rev133E.
        geo = test_data_dir / "Rev133E_X43_CRing_GEO.TAB"
        cal = test_data_dir / "Rev133E_X43_CRing_CAL.TAB"
        dlp = test_data_dir / "Rev133E_X43_CRing_DLP_500M.TAB"
        tau = test_data_dir / "Rev133E_X43_CRing_TAU_1000M.TAB"

        # Verify that the test data files exist
        assert geo.exists(), f"'{geo}' does not exist"
        assert cal.exists(), f"'{cal}' does not exist"
        assert dlp.exists(), f"'{dlp}' does not exist"
        assert tau.exists(), f"'{tau}' does not exist"
        ...
        res_label = "%s km" % str(TauInst.res)

        # Total number of plots, and total number of pages.
        N_Plots = len(reg_dict)
        ...
        with PdfPages(str(outfile)) as pdf:
            for i_page in range(N_Pages):
                ...
                plt.suptitle(f"{rev} Reconstructions: {res_label} Resolution", size=14)
                ...
                for i_plot in range(8):
                    ...
                    plt.tick_params(
                        axis="y", which="both", left=True, right=True, labelleft=True
                    )
                    plt.tick_params(
                        axis="x", which="both", bottom=True, top=True, labelbottom=True
                    )
                    ...
                    plt.title(reg_dict[int(8 * i_page + i_plot)][0])
                    ...
                    rmin = reg_dict[int(8 * i_page + i_plot)][1] - 25.0
                    rmax = reg_dict[int(8 * i_page + i_plot)][1] + 25.0
            ...
            if (N_Plots % 8) != 0:
                ...
                plt.figure(figsize=(8.5, 11))
                plt.suptitle(
                    f"{rev} C-Ring Reconstructions: {res_label} Resolution", size=14
                )
                ...
                for i_plot in range(int(N_Plots % 8)):
                    ...
                    plt.title(reg_dict[int(8 * (N_Pages - 1) + i_plot)][0])
                    ...
                    rmin = reg_dict[int(8 * (N_Pages - 1) + i_plot)][1] - 15.0
                    rmax = reg_dict[int(8 * (N_Pages - 1) + i_plot)][1] + 15.0
        ...
        assert outfile.exists(), "PDF was not created"

    def test_ModelFromGEO(
        self, test_root_dir: Path, test_data_dir: Path, figures_dir: Path
    ):
        # Location of the data for Rev007E.
        geo = test_data_dir / "Rev007E_X43_Maxwell_GEO.TAB"
        cal = test_data_dir / "Rev007E_X43_Maxwell_CAL.TAB"
        dlp = test_data_dir / "Rev007E_X43_Maxwell_DLP_500M.TAB"
        ...
        # Name of the output PDF file.
        outfile = figures_dir / "ModelFromGEO_Test.pdf"
        if outfile.exists():
            # Remove the previous copy
            outfile.unlink()
        ...
        with PdfPages(str(outfile)) as pdf:
            ...
            for model in [
                ...
                for opacity in {0.0, 0.3}:
                    ...
                    if (opacity != 0) and (
                        (model == "Square Wave") or (model == "Delta Impulse")
                    ):
                        continue

                    # Create the modeled data without the Fresnel approximation.
                    gdata = at.ModelFromGEO(
                        str(geo),
                        wav,
                        res,
                        rho,
                    ...
                    # Title for the page.
                    plt.suptitle(
                        "ModelFromGEO - %s - 200m Model - Opacity: %.1f"
                        % (model, opacity),
                        size=14,
                    )
                    ...
                    plt.plot(
                        gdata.rho_km_vals, gdata.p_norm_actual_vals, "g", label=lba
                    )
            ...
            data = ExtractCSVData(str(geo), str(cal), str(dlp), verbose=False)
            ...
            plt.plot(gdata.rho_km_vals, gdata.p_norm_vals, "r", label="Modeled Data")
            ...
            # Plot the reconstructed power.
            plt.plot(rec.rho_km_vals, rec.power_vals, "r", label="Real Reconstruction")
            plt.plot(
                grec.rho_km_vals,
                grec.power_vals,
                "b",
    ...
    def test_Make_MTR_Figs(
        ...
        geo = test_data_dir / "VG1_SRING_GEO_20191119.TAB"

        # Verify that the test data file exists
        assert geo.exists(), f"{str(geo)} was not found"
        ...
        # Name of the output PDF file.
        outfile = figures_dir / "MTR_Figs.pdf"
        if outfile.exists():
            # Remove the previous copy
            outfile.unlink()
        ...
        with PdfPages(str(outfile)) as pdf:
            ...
            # Title for the page.
            plt.suptitle(
                "MTR Fig 5 - 1km Resolution - Maxwell Ringlet - Rev007E", size=14
            )
