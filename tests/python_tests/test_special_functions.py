...
#
#         # Compute approximate Fresnel transform for current point
#         T_out[center] = numpy.sum(ker * T_in[crange]) * dx_km * (0.5+0.5j) / F
#
#         # If normalization has been set, normalize the reconstruction.
#         if norm:
#             T_out[center] *= window_functions.window_norm(dx_km, ker, F)
...
#     cosp = numpy.cos(phi_rad_vals)
#     sinp = numpy.sin(phi_rad_vals)
...
#     A_2 = 0.5*numpy.square(cosb*sinp)/(1.0-numpy.square(cosb*sinp))
#
...
#     fresnel_p = []
#     fresnel_p.append(0.5-0.5*numpy.square(legendre_p[1]))
#     coeffs_p = []
#     l_coeffs = []
#     for i in range(ord):
...
#
#     i = int((ord+1)/2)
#     coeffs_p.append((fresnel_p[ord-1]-Legendre_Coeff*numpy.square(legendre_p[i])))
#     rcpr_d = 1.0/D_km_vals
#     rcpr_d2 = numpy.square(rcpr_d)
#
#     # Perform Fresnel transform, point by point, using Riemann sums.
#     for i in numpy.arange(n_used):
#
...
#         psi_vals *= kD_vals[center] * numpy.square(z)
#
#         # Compute kernel function for Fresnel inverse or forward model.
#         if fwd:
#             ker = w_func*numpy.exp(1j*psi_vals)
