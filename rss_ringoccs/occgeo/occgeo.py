...
class Geometry:
    ...
    def __init__(
        ...
        if year >= 2016:
            ...
            if (mask == True).all() == False:
                ...
                if verbose:
                    print(
                        "\tRemoving false intercept points: "
                        + "Indices "
                        + str(inds[0])
                        + " to "
                        + str(inds[-1])
        ...
        if year >= 2011:
            ...
            (
                ...
                t_ul_ret_vals,
                self.ul_rho_km_vals,
                self.ul_phi_rl_deg_vals,
                self.ul_phi_ora_deg_vals,
            ) = cog.calc_uplink_geo(
                ul_dsn_used, t_set_et_vals, spacecraft, planet, nhat_p, ref=ref
            )
    ...
    def __remove_values_beyond_rings(self):
        ...
        ind1 = np.argwhere(self.rho_km_vals > 170000.0)[0][0]
    ...
    def __remove_atmos_values(self):
        ...
        self.add_info["Blocked by atmosphere"] = (
            ...
            + str(ind2)
            + ", or OET from "
            + str(self.t_oet_spm_vals[ind1])
            + " to "
            + str(self.t_oet_spm_vals[ind2])
            + " SPM"
        )
