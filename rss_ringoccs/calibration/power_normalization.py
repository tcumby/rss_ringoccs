...
class Normalization:
    ...
    def __init__(
        ...
        if no_gaps:
            ...
            if (self.mask == False).all():
                ...
                # Compute fit
                self.fit_freespace_power(
                    spm_down, p_obs_down, order=self.order, fittype=fittype
                )

        # get User input to see if everything looks okay or needs tweaking
        #    will return modified freespace regions
        if interact:
            new_gaps = self.fit_check(spm_down, p_obs_down, self.gaps, self.order)
        else:
        ...
        if write_file:
            self.plot_power_profile(
                spm_down, p_obs_down, new_gaps, self.order, save=True
            )
    ...
    def fit_check(self, spm_down, p_obs_down, freespace_spm, order):
        ...
        while "n" in cont or "N" in cont:
            ...
            if "y" in change_freespace or "Y" in change_freespace:
                ...
                revert_gaps = input(
                    "Do you want to revert to the default " + "freespace gaps? "
                )
                ...
                # if not reverting, prompt for new freespace regions
                    ...
                    new_gaps_str = input(
                        "Please input new freespace gaps in "
                    ...
                    if "[[" not in new_gaps_str and "]]" not in new_gaps_str:
                        print(
                            "Invalid entry for freespace gaps.\nMust be Nx2 " + "list."
                        )
            ...
            # Prompt for fit type
            print("\n")
            fittype = input("What fit type would you like to use? " + "(poly/spline): ")
    ...
    def create_mask(self, spm, gaps_spm, pc):
        ...
        if len(gaps_spm) == 2 or len(gaps_spm) == 3:
            if gaps_spm[0][1] == gaps_spm[-1][0]:
                pc_max = np.nanmax(
                    pc[(spm >= gaps_spm[0][0]) & (spm <= gaps_spm[-1][-1])]
                )
            else:
                pc_max = np.nanmax(
                    pc[(spm >= gaps_spm[0][1]) & (spm <= gaps_spm[-1][0])]
                )
        elif len(gaps_spm) == 1:
    ...
    def fit_freespace_power(self, spm, power, order=3, fittype="poly"):
        ...
        if fittype == "poly":
            ...
            chi2 = (
                np.sum(np.square(np.polyval(coef, spm[self.mask]) - power[self.mask]))
                / v
            )
        ...
        # spline fit
            ...
            coef = splrep(
                spm[self.mask], power[self.mask], k=order, t=knots_spm, s=smooth
            )
            ...
            fit = np.polyval(coef, spm)
            chi2 = np.sum(
                np.square(np.polyval(coef, spm[self.mask]) - power[self.mask])
                / np.polyval(coef, spm[self.mask])
            )
    ...
    def plot_power_profile(self, spm, pow, gaps, order, save=False):
        ...
        ax_prof.plot(
            spm[self.mask] / 1e3 - spm_off, pow[self.mask] / pmax, ".k", ms=6, zorder=1
        )
        ...
        for i in range(len(ax)):
            ...
            if i < len(gaps):
                ...
                # plotting
                ax[i].plot(
                    spm[self.mask] / 1e3 - spm_off, pow[self.mask] / pmax, ".k", ms=6
                )
        ...
        plt.subplots_adjust(
            wspace=None, hspace=0.3, left=0.1, right=0.975, top=0.95, bottom=0.075
        )
        # axis labels
    ...
    def downsample_IQ(self, spm_raw, IQ_c_raw, dt_down=0.5, verbose=False):
        ...
        spm_vals_down = np.linspace(spm_raw[0], spm_raw[-1], num=len(p_obs_down))
