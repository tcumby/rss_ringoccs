...
class DiffractionLimitedProfile:
    """
    :Purpose:
    ...
    def __init__(
        ...
        verbose=False,
        write_file=True,
        profile_range=[65000.0, 150000.0],
    ):
        ...
        if (spm_full > max(spm_geo)).any() or (spm_full < min(spm_geo)).any():
        ...
        rho_km_desired, IQ_c_desired = resample_IQ(
            rho_full, IQ_c, dr_km, verbose=verbose
        )
        ...
        if (
            np.logical_and(
                rho_km_desired > profile_range[0], rho_km_desired < profile_range[1]
            ).any()
            == False
        ):
            minmax = [min(rho_km_desired), max(rho_km_desired)]
            raise ValueError(
                "(dlp_class.py): Data does not cover desired "
        ...
        ind1 = list(rho_km_desired).index(
            min(list(rho_km_desired), key=lambda x: abs(x - profile_range[0]))
        )

        ind2 = list(rho_km_desired).index(
            min(list(rho_km_desired), key=lambda x: abs(x - profile_range[1]))
        )
        ...
        if min(p_norm_vals) < 0.0:
            raise ValueError(
                "(dlp_class.py): Negative power values found " + "within ring system!"
            )

        phase_rad_vals = np.arctan2(np.imag(IQ_c_desired), np.real(IQ_c_desired))
        ...
        if hasattr(geo_inst, "ul_rho_km_vals"):
            ...
            self.t_ul_spm_vals = np.interp(self.t_oet_spm_vals, spm_geo, t_ul_spm_vals)
            self.t_ul_ret_spm_vals = np.interp(
                self.t_oet_spm_vals, spm_geo, t_ul_ret_spm_vals
            )
            self.ul_rho_km_vals = np.interp(
                self.t_oet_spm_vals, spm_geo, ul_rho_km_vals
            )
            self.ul_phi_rl_deg_vals = np.interp(
                self.t_oet_spm_vals, spm_geo, ul_phi_rl_deg_vals
            )
            self.ul_phi_ora_deg_vals = np.interp(
                self.t_oet_spm_vals, spm_geo, ul_phi_ora_deg_vals
            )
        ...
        input_kwds = {"profile_range": profile_range}

        if self.add_info == {}:
            self.add_info = None

        self.history = write_history_dict(
            input_vars, input_kwds, __file__, add_info=self.add_info
        )
    ...
    def __interp_and_set_attr(
        ...
        if min(rho_dot_kms_vals_interp) < 0 and max(rho_dot_kms_vals_interp) > 0:
            ...
            self.add_info["Interpolating past prof_dir"] = (
                ...
                + str(ind[-1])
                + ", or OET from "
                + str(spm_desired[ind[0]])
                + " to "
                + str(spm_desired[ind[-1]])
                + " SPM"
            )
        ...
        phi_ora_rad_vals_interp = np.interp(spm_desired, spm_geo, phi_ora_rad_geo)

        # obvserved ring azimuth at final spacing
        # spm_to_phi_rl = splrep(spm_geo, phi_rl_rad_geo, k=1)
        # phi_rl_rad_vals_interp = splev(spm_desired, spm_to_phi_rl)
        phi_rl_rad_vals_interp = np.interp(spm_desired, spm_geo, phi_rl_rad_geo)
        ...
        self.raw_tau_threshold_vals = np.interp(spm_desired, spm_thresh, tau_thresh)
    ...
    def create_dlps(
        ...
        if prof_dir == '"INGRESS"':
            ...
            ind = np.argwhere(np.diff(np.sign(geo_inst.rho_dot_kms_vals)))[0][0] + 1
            ...
            ind_geo_ing = np.argwhere(geo_inst.t_oet_spm_vals == (ind_spm1 - 1.0))[0][0]
            ind_rsr_ing = np.argwhere(rsr_inst.spm_vals == (ind_spm1 - 1.0))[0][0]
            ind_cal_ing = np.argwhere(cal_inst.t_oet_spm_vals == (ind_spm1 - 1.0))[0][0]

            # For egress occ, remove 1s from the beginning
            ind_geo_egr = np.argwhere(geo_inst.t_oet_spm_vals == (ind_spm1 + 1.0))[0][0]
            ind_rsr_egr = np.argwhere(rsr_inst.spm_vals == (ind_spm1 + 1.0))[0][0]
            ind_cal_egr = np.argwhere(cal_inst.t_oet_spm_vals == (ind_spm1 + 1.0))[0][0]
            ...
            cal_ing.f_offset_fit_vals = cal_inst.f_offset_fit_vals[:ind_cal_ing]
            ...
            cal_egr.f_offset_fit_vals = cal_inst.f_offset_fit_vals[ind_cal_egr:]
            ...
            raise ValueError(
                "ERROR (create_dlps()): Invalid profile " + "direction given!"
            )
