...
class FreqOffsetFit:
    ...
    def __init__(
        ...
        if not isinstance(rsr_inst, RSRReader):
            sys.exit(
                "ERROR (FreqOffsetFit): rsr_inst input must be an "
                + "instance of the RSRReader class"
            )

        if not isinstance(geo_inst, Geometry):
            sys.exit(
                "ERROR (FreqOffsetFIt): geo_inst input must be an "
                + "instance of the Geometry class"
            )

        if not isinstance(verbose, bool):
            print(
                "WARNING (FreqOffsetFit): verbose input must be boolean. "
        ...
        if self.band == "X":
            ...
            raise ValueError(
                "WARNING (freq_offset_fit.py): Invalid frequency " + "band!"
            )
        ...
        if (
            fof_lims != None
            and len(fof_lims) == 2
            and fof_lims[1] - fof_lims[0] > 100.0
        ):
            spm_min = fof_lims[0]
            spm_max = fof_lims[1]
        ...
        f_sky_recon = calc_f_sky_recon(f_spm, rsr_inst, sc_name, f_uso, kernels)
    ...
    def create_mask(self, f_spm, f_rho, f_offset, polyclip=False, Cval=7.5):  # 12.5):
        ...
        """
        dt_spm = round(
            (self.raw_spm_vals[-1] - self.raw_spm_vals[0])
            / float(len(self.raw_spm_vals)),
            8,
        )
        ...
        f_stdev = 5.0 * np.sqrt(np.nanmedian(np.square(f_offset - f_median)))
        if f_stdev > 15.0:
            f_stdev = 15.0
        ...
        if polyclip:
            for ipol in range(3):
                ...
                #   for data which fall in acceptable regions
                fit_stdev = 5.0 * np.sqrt(
                    np.nanmedian(np.square(f_offset - np.polyval(pinit, f_spm)))
                )
        ...
        for i in range(len(f_offset)):
            ...
            # exclude data with variability greater than df
                ...
                # proceed to check variability within the masked data
                    ...
                    if (
                        (i + j) < (i + j + k)
                        and (i + j) < len(f_offset)
                        and (i + j + k) < len(f_offset)
                    ):
                        # if both checks pass
                        if chk3 and chk4:
    ...
    def calc_poly_order(self, f_spm_cl, f_offset_cl, verbose=False, max_order=9):
        ...
        while F > 1.0 and polyord2 < max_order + 1:
    ...
    def get_disconts(self, f_spm, f_rho, f_offset, df=0.1):
        ...
        for i in range(1, len(disconts) - 1):
            ...
            abdif = [
                abs(
                    f_offset[self.__mask][disconts[i]]
                    - f_offset[self.__mask][disconts[i] - 1]
                ),
                abs(
                    f_offset[self.__mask][disconts[i] + 1]
                    - f_offset[self.__mask][disconts[i]]
            ...
            check4 = (f_rho[self.__mask][disconts[i]] > 9.1e4) & (
                f_rho[self.__mask][disconts[i]] < 1.19e5
            )
    ...
    def __centdiff2(self, x, f):
        ...
        h2 = (xi - xb) * (
            xf - xi
        )  # h squared as backward difference * forward difference
    ...
    def fit_freq_offset(self, f_spm, f_rho, f_offset, verbose=False):
        ...
        if self.year < 2011:
            ...
            poly_order = self.calc_poly_order(
                spm_temp[self.__mask], f_offset[self.__mask]
            )
            ...
            chi2 = np.sum(
                np.square(f_offset_fit[self.__mask] - f_offset[self.__mask])
                / f_offset_fit[self.__mask]
            )
