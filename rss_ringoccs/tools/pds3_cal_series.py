...
def get_cal_series_info(rev_info, cal_inst, series_name, prof_dir):
    ...
    geo_kernels = cal_inst.history["Positional Args"]["geo_inst"]["Positional Args"][
        "kernels"
    ]
    ...
    FILE_DESCRIPTION = (
        ...
        + sd
        + "reliable optical depth profiles are computed. The "
        + "attributes are the"
        + sd
        + "signal sky-frequency, the "
        + "frequency offset, and the free-space"
        + sd
        + "signal power. The attributes are listed versus "
        "OBSERVED " + "EVENT TIME"
        ""
        + sd
        + "(Earth received time) over equal "
        + "time increments (1 s)."
        + sd
        + " "
        + sd
        ...
        + "frequency"
        + sd
        + "of the downlink sinusoid to a constant "
        + "value (near the center of the"
        + sd
        + "recording bandwidth) before the sinusoid amplitude and "
        + "phase can be"
        + sd
        + "estimated. The free-space signal "
        + "power estimates are required to"
        + sd
        + "normalize the "
        + "power of the steered sinusoid to value of about unity"
        + sd
        + "(optical depth of about zero) outside Ring A, inside Ring C, "
        + "and within"
        + sd
        + "large ring gaps. The sky-frequency "
        + "estimates are included for completeness"
        + sd
        ...
        + "the (RSR)"
        + sd
        + "steers the frequency of the received "
        + "sinusoid so that the measured"
        + sd
        + "spectral line falls "
        + "at the center of the recording bandwidth. Spectral"
        + sd
        + "estimates of the measured I/Q samples can be used to "
        + "calculate any"
        + sd
        + "offsets of the spectral line from "
        + "the center of the bandwidth. The"
        + sd
        + "measured offsets together with other frequency steering "
        + "information"
        + sd
        + "encoded in the RSR recording are used "
        + "to calculate the listed sky-"
        + sd
        + "frequency based on procedures documented in the JPLD-16765. "
        + "Part"
        + sd
        + "of the measured frequency offset is caused "
        + "by the use of a predicted"
        + sd
        + "spacecraft trajectory to estimate "
        + "the received Doppler-shifted signal"
        + sd
        + "frequency needed to steer the RSR. This part can be removed "
        + "by using"
        + sd
        + "the more accurate trajectory reconstructed "
        + "by the Cassini Navigation"
        + sd
        + "Team instead. "
        + "The listed frequency offset is the polynomial fit "
        + "over the"
        + sd
        + "global extent of the rings. The free-space "
        + "signal power estimates are"
        + sd
        + "also calculated using "
        + "global polynomial fits to power estimates outside"
        + sd
        ...
        + "Cassini"
        + sd
        + "Navigation Team Naif Toolkit kernel files "
        + "available at the time of"
        + sd
        + "archiving and are listed "
        + "above. We note that the adopted Planetary"
        + sd
        + "Constants Kernel (PCK) file is not necessarily the one "
        + "Cassini NAV"
        + sd
        + "associates with the listed "
        + "reconstructed trajectory file. The"
        + sd
        + "difference this causes to estimate ring radius is well "
        + "below 1 km"
        + sd
        + "and has negligble impact on the "
        + "archived products."
        + sd
        + " "
        + sd
        ...
        + "the USO."
        + sd
        + "Typical values of the Allan deviation "
        + "is 2E-13 over 1 s and 1E-13"
        + sd
        + "over 10-100 s. "
        + "The value changed little over the USO lifetime."
        + sd
        + " "
        + sd
    ...
    SERIES_DESCRIPTION = (
        ...
        + "power as a"
        + sd
        + "function of OBSERVED EVENT TIME "
    ...
    object_descriptions = [
        (
            ...
            + "seconds"
            + sd
            + "after the moment specified by "
            + "REFERENCE_TIME. Also referred to"
            + sd
            + 'as Earth receiving time or ERT."'
        ),
        (
            '"Frequency of the downlink sinusoidal'
            + sd
            + "signal "
            ...
            + "recorded by the RSR and the frequency"
            + sd
            + "offset from the center of the recording bandwidth "
            + "observed in"
            + sd
            + "computed power spectra of the "
            + "measured I/Q samples. See formula"
            + sd
            + 'given in Section 2.6 of JPLD-16765."'
        ),
        (
            '"The offset frequency is the'
            + sd
            + "frequency of the "
            + "carrier signal relative to the"
            + sd
            + 'center of the receiver bandwidth."'
        ),
        (
            '"Estimate of the power of the received'
            + sd
            + "sinusoid "
            + "in the absence of the rings. It is calculated using"
            + sd
            + "weighted least-square spline fit to estimates of the "
            + "downlink"
            + sd
            + "carrier power in regions outside of "
            + "Ring A and inside of Ring C"
            + sd
            + "(when feasible), "
            + "and within prominent gaps in Ring A, the Cassini"
            + sd
            + "Division, and Ring C. Time history of the estimate is "
            + "used to"
            + sd
            + "normalize the free-space power of the "
            + "downlink signal to unity"
            + sd
            + "across the extent "
            + 'of the ring system."'
        ),
