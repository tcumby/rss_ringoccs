...
import numpy as np
import time
import rss_ringoccs.tools.pds3_write_series_v2 as pds3
...
def get_tau_series_info(rev_info, tau_inst, series_name, prof_dir):
    ...
    RADIAL_RESOLUTION = (
        str(float(pds3.get_sampling_interval(sampling_parameter_arr)) * 2.0) + "   <km>"
    )
    RADIAL_SAMPLING_INTERVAL = (
        pds3.get_sampling_interval(sampling_parameter_arr) + "   <km>"
    )
    ...
    MINIMUM_RING_LONGITUDE = (
        str(round(min(np.degrees(tau_inst.phi_rl_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_RING_LONGITUDE = (
        str(round(max(np.degrees(tau_inst.phi_rl_rad_vals)), 4)) + "   <deg>"
    )
    MINIMUM_OBSERVED_RING_AZIMUTH = (
        str(round(min(np.degrees(tau_inst.phi_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_OBSERVED_RING_AZIMUTH = (
        str(round(max(np.degrees(tau_inst.phi_rad_vals)), 4)) + "   <deg>"
    )
    MINIMUM_OBSERVED_RING_ELEVATION = (
        str(round(min(np.degrees(tau_inst.B_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_OBSERVED_RING_ELEVATION = (
        str(round(max(np.degrees(tau_inst.B_rad_vals)), 4)) + "   <deg>"
    )
    ...
    RADIAL_RESOLUTION_dict = {
        ...
        "MINIMUM_OBSERVED_RING_ELEVATION": MINIMUM_OBSERVED_RING_ELEVATION,
        "MAXIMUM_OBSERVED_RING_ELEVATION": MAXIMUM_OBSERVED_RING_ELEVATION,
    ...
    FILE_DESCRIPTION = (
        '"The '
        "TAU"
        " file contains calibrated"
        + sd
        + "optical depth and phase shift profiles of Saturn"
        "s rings "
        + "reconstructed from"
        + sd
        + "the diffraction-limited "
        + "measurements. The frequency/phase measurements"
        + sd
        + "reference is the Cassini UltraStable Oscillator (USO). "
        + "The reconstruction"
        + sd
        + "is carried out using algorithms "
        + "described at length in MAROUFETAL1986."
        + sd
        ...
        + "RING_OBSERVATION_ID"
        + sd
        + "(listed in the header of the "
        + "LBL file) as this product. These include"
        + sd
        + "one or more reconstructed profiles (TAU) files for the same "
        + "occultation but"
        + sd
        + "at different resolutions, and two "
        + "which provide geometry and calibration"
        + sd
        + "data. The latter two have file names constructed from the same "
        + "root as"
        + sd
        + "this file with the field for radial "
        + "resolution removed and the "
        "TAU"
        "" + sd + "replaced by either" + " "
        "GEO"
        " (geometry) or "
        "CAL"
        ...
        + "profiles"
        + sd
        + "(DLP) files are also included. In such "
        + "cases, the DLP file name lists the"
        + sd
        + "DLP resolution and "
        "TAU"
        " is replaced by "
        ...
        + "conditions)"
        + sd
        + "and 10 km resolution reconstructed "
        + "profiles (TAU) files are included. We"
        + sd
        + "define spatial resolution as the shortest resolvable "
        + "wavelength in a"
        + sd
        + "reconstructed profile. The definition "
        + "is convenient for characterization"
        + sd
        + "of multiple resolution profiles generated from a "
        + "reference reconstructed"
        + sd
        + "profile of resolution "
        + "much finger than 1 km (when experimental conditions"
        + sd
        + "allows). The shortest resolvable wavelength is the inverse "
        + "of the highest"
        + sd
        + "spatial frequency preserved in the "
        + "data and is determined by the bandwidth"
        + sd
        + "of the lowpass filter applied at the end of the data "
        + "processing chain. The"
        + sd
        + "archived 1 km resolution "
        + "profile is identical to one reconstructed directly"
        + sd
        ...
        + "including stability of the reference"
        + sd
        + "oscillator. The latter is usually charactereized by the Allan"
        + sd
        + "deviation/variance. Typical values of the Cassini USO "
        + "Allan deviation is"
        + sd
        + "2E-13 over 1s and 1E-13 over "
        + "10-100s. The USO Allan deviation changed little"
        + sd
        + "over the USO lifetime. Cassini USO stability does not "
        + "measurably impact"
        + sd
        + "reconstruction resolution "
        + "down to the meters range."
        + sd
        + " "
        + sd
        ...
        + "reference"
        + sd
        + "frequency of 8,427,222,034.34050 Hz "
        + "at X-band, its value near the"
        + sd
        + "beginning of the "
        ...
        + "USO"
        + sd
        + "frequency changed slightly (at the Hz level) "
        + "during the USO lifetime."
        + sd
        + "The change negligibly "
        + "impacts the archived products."
        + sd
        + " "
        + sd
        ...
        + "processing suite"
        + sd
        + "developed at Wellesley College with "
        + "the support of the Cassini project and"
        + sd
        + "hosted "
        ...
        + "Richard G. French,"
        + sd
        + "Astronomy Department, Wellesley College;"
        + sd
    ...
    HIST_description = (
        "This is a record of the processing steps"
        + sd
        + "and inputs used to generate this file."
    )
    ...
    SERIES_DESCRIPTION = (
        '"This series contains variations of the'
        + sd
        + "optical depth and phase shift profiles as a function of "
        + "RING RADIUS. Also"
        + sd
        + "included are"  # additive radius '
        # + 'correction terms, optical depth threshold,' + sd
        + ' some pertinent event times, and geometry parameters."'
    )
    SERIES_USAGE_NOTE = (
        '"This product was made as a cross-check to'
        + sd
        + "Cassini_RSS_Ring_Profiles_2018_Archive using an "
        + "independent processing"
        + sd
        + "software. There are two columns "
        ...
        + "and"
        + sd
        + "are meant only as placeholders to match the "
        + "file contents of"
        + sd
    ...
    SERIES_dict = {
        ...
        "MINIMUM_SAMPLING_PARAMETER": SERIES_MINIMUM_SAMPLING_PARAMETER,
        "MAXIMUM_SAMPLING_PARAMETER": SERIES_MAXIMUM_SAMPLING_PARAMETER,
        "SAMPLING_PARAMETER_INTERVAL": SERIES_SAMPLING_PARAMETER_INTERVAL,
    ...
    object_descriptions = [
        ...
        (
            '"This is a placeholder column to match' + sd + "the "
            "DLP"
            " file format provided in: Cassini_RSS_Ring_"
            + "Profiles_2018_Archive."
            + sd
        ...
        (
            '"Inertial (J2000) longitude in the ring'
            + sd
            + "plane of the ring-plane intercept point at the "
            + 'RING EVENT TIME."'
        ),
        (
            '"Measured at the ring-plane intercept'
            + sd
            + "point, "
            + "starting from the direction of a photo heading to the"
            + sd
            + "observer (Earth receiving station), and ending at the "
            + "direction of"
            + sd
            + "a local radial vector. This angle "
            + "is projected into the ring plane"
            + sd
            + "and measured "
            + "in the prograde direction. As seen from the observer,"
            + sd
            + "it equals 90 degrees along the ring ansa and 270 degrees "
            + "along the"
            + sd
            + "left ansa. Values range from 0 to 360 "
            + "degrees. This convention for"
            + sd
            + "observed ring "
            + "azimuth differs from that adopted in MAROUFETAL1986 by"
            + sd
            + '180 degrees."'
        ),
        (
            '"Power (amplitude square) of the'
            + sd
            + "ring-attenuated "
            + "reconstructed radio signal, normalized by its value in"
            + sd
            + "the absence of the rings (normalized to unity in "
            + "free-space). The"
            + sd
            + "value may be used to compute "
            + "the reconstructed oblique optical depth"
            + sd
            + "as the negative natural logarithm of the NORMALIZED "
            + 'SIGNAL POWER."'
        ),
        (
            '"The normal optical depth obtained from its'
            + sd
            + "measured oblique value scaled by the sine of the absolute "
            + "value of ring"
            + sd
            + "opening angle (OBSERVED RING ELEVATION). "
            + "The oblique value is computed"
            + sd
            + "from reconstructed "
            + "complex ring transmittance; see Eq. (21) of"
            + sd
            + 'MAROUFETAL1986."'
        ),
        (
            ...
            + sd
            + "signal) and its value had the rings been absent. The "
            + "phase shift is"
            + sd
            + "computed from the reconstructed "
            + "complex ring transmittance;"
            + sd
            + "see Eq. (22) of MAROUFETAL1986. The phase reference "
            + "of the original"
            + sd
            + "measurements is the Cassini "
            + 'UltraStable Oscillator, or USO."'
        ),
        (
            '"The value of the normal optical depth'
            + sd
            + "for which "
            + "the magnitude of the reconstructed complex ring transmittance"
            + sd
            + "is equal in numerical value to the 70% confidence "
            + "interval of the"
            + sd
            + "noisy measurement (signal-to-"
            + "noise ratio, or SNR, of about unity)."
            + sd
            + "See Eq. (26) of MAROUFETAL1986. The threshold value "
            + "and corresponding"
            + sd
            + "LOWEST_DETECTABLE_OPACITY "
            + "and HIGHEST_DETECTABLE_OPACITY are based on"
            + sd
            + "the measurement SNR as defined in MAROUFETAL, Eq. (27), "
            + "and include"
            + sd
            + "only the contribution of thermal "
            + "noise. Further limitations on the"
            + sd
            + "lowest "
            + "detectable optical depth may be imposed by ground antenna"
            + sd
            + "pointing errors. The imposed limitations may be "
            + "assessed from"
            + sd
            + " examination of the optical depth "
        ...
        (
            '"The instant at which photons were'
            + sd
            + "received at "
            + "the DSN receiving station, given in elapsed seconds"
            + sd
        ...
        (
            '"The time at which photos left the'
            + sd
            + "ring plane. "
            + "This time is earlier than the associated"
            + sd
            + "OBSERVED EVENT TIME by an amount equal to the light travel "
            + "time."
            + sd
            + "RING EVENT TIME is given in elapsed seconds "
            + "after the moment"
            + sd
        ...
        (
            ...
            + sd
            + 'REFERENCE_TIME. Also referred to as SCET."'
        ),
        (
            '"The angle measured at the ring'
            + sd
            + "intercept point, "
            + "starting from the ring plane and ending in the"
            + sd
            + "direction of the photon heading toward the observer. "
            + "This angle is"
            + sd
            + "positive on the north side of "
            + "Saturn"
            "s rings and negative on the"
            + sd
            + "south side. "
            + "Its value is nearly constant over the duration of a "
            + sd
            + "ring occultation experiment and is nearly equal to the "
            + "ring opening"
            + sd
            + "angle (Earth elevation angle "
    ...
    if hasattr(tau_inst, "ul_rho_km_vals"):
        ...
        object_descriptions.append(
            '"The time at which photos were transmitted from'
            + sd
            + "the uplink DSN station, given in elapsed seconds after"
            + "the moment specified"
            + sd
            + "by REFERENCE_TIME. This time "
            + "is earlier than the SPACECRAFT EVENT TIME by"
            + sd
        ...
        reference_times.append(OBJECT_REFERENCE_TIME)
        object_descriptions.append(
            '"The time at which photons transmitted by the'
            + sd
        ...
        object_descriptions.append(
            '"Radial distance from the center of Saturn to'
            + sd
        ...
        reference_times.append(es)
        object_descriptions.append(
            '"Inertial (J2000) longitude in the ring plane'
            + sd
        ...
        reference_times.append(es)
        object_descriptions.append(
            '"Measured at the uplink ring-plane intercept'
            + sd
            + "point, starting from the "
            + "direction of a photon heading to the spacecraft"
            + sd
            + "and ending at the direction of a local radial vector. "
            + "This angle is"
            + sd
            + "projected into the ring plane and "
