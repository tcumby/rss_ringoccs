...
def get_geo_series_info(rev_info, geo_inst, series_name, prof_dir):
    ...
    FILE_DESCRIPTION = (
        ...
        + sd
        + "reconstruction of "
        + "the diffraction limited profiles and for other"
        + sd
        + "data analysis purposes. Light-time effects have been "
        + "accounted for"
        + sd
        + "in computing these parameters. "
        + "For more details, see MAROUFETAL1986"
        + sd
        + "and Appendix B of ROSEN1989."
        + sd
        + " "
        + sd
        ...
        + " Cassini"
        + sd
        + "Navigation Team NAIF Toolkit kernel "
        + "files available at the time of"
        + sd
        + "archiving and are listed above. We note that the adopted "
        + "Planetary"
        + sd
        + "Constants Kernel (PCK) file is not "
        + "necessarily the one Cassini NAV"
        + sd
        + "associates with the listed reconstructed trajectory file. The"
        + sd
        + "difference this causes to estimated ring radius is well "
        + "below 1 km and"
        + sd
        + "has negligible impact on the archived "
        + "products."
        + sd
        + " "
        + sd
        ...
        + "reference"
        + sd
        + "frequency of 8,427,222,034.34050 Hz at "
        + "X-band, its value near the"
        + sd
        + "beginning of the Cassini orbital tour. The frequency is "
        + "coherently"
        + sd
        + "scaled by 3/11 for S-band and 209/55 "
        + "for Ka-band. The exact USO"
        + sd
        + "requency changed slightly (at the Hz level) during the USO "
        + "lifetime."
        + sd
        + "The change negligibly impacts the "
        + "archived products. The same holds"
        + sd
        + "true for the Allan deviation characterizing the stability "
        + "of the USO."
        + sd
        + "Typical values of the Allan deviation is "
        + "2E-13 over 1 s and 1E-13 over"
        + sd
        + "10-100 s. "
        + "The values changed little over the lifetime of the USO."
        + sd
        + " "
        + sd
    ...
    SERIES_DESCRIPTION = (
        '"Series contains ring occultation'
        + sd
        + "geometry data needed for diffraction reconstruction "
        + "of the rings"
        + sd
        + "optical depth profile and for "
    ...
    object_descriptions = [
        (
            ...
            + "elapsed seconds"
            + sd
            + "after the moment specified "
            + "by REFERENCE_TIME. Also referred to"
            + sd
        ...
        (
            ...
            + "light travel time."
            + sd
            + "RING EVENT TIME is given "
            + "in elapsed seconds after the moment"
            + sd
        ...
        (
            ...
            + "the"
            + sd
            + "observer (Earth receiving station), "
            + "and ending at the direction of"
            + sd
            + "a local radial vector. This angle is "
            + "projected into the ring plane"
            + sd
            + "and measured in the "
            + "prograde direction. As seen from the observer, "
            + sd
            + "it equals 90 degrees along the right ansa and 270 degrees "
            + "along the"
            + sd
            + "left ansa. Values range from 0 "
            + "to 360 degrees. This convention for"
            + sd
        ...
        (
            ...
            + sd
            + "direction of the "
            + "photon heading toward the observer. This angle is"
            + sd
            + "positive on the north side of Saturn"
            "s rings and "
            + "negative on the"
            + sd
            + "south side. Its value is "
            + "nearly constant over the duration of a"
            + sd
            + "ring occultation experiment and is nearly equal to the "
            + "ring opening"
            + sd
            + "angle (Earth elevation angle above "
            + 'the ring plane)."'
        ),
        (
            '"The distance between the spacecraft (at'
            + sd
            + "the SPACECRAFT EVENT TIME) and the ring-plane "
            + "intercept point (at the"
            + sd
            + 'RING EVENT TIME)."'
        ),
        (
            '"Component of the displacement velocity'
            + sd
        ...
        (
            ...
            + "(at the RING EVENT TIME). The orbital direction"
            + sd
            + "is defined by the cross-product of a vector along "
            + "Saturn"
            "s pole and"
            + sd
            + "the radial velocity at "
            + 'the intercept point, in that order."'
        ),
        (
            '"Fresnel scale of diffraction implied by'
            + sd
            + "the occultation geometry at X-band. See Eq. (6) of "
            + "MAROUFETAL1986."
            + sd
            + "The Fresnel scale at S-band is "
            + "sqrt(11/3) times FRESNEL SCALE."
            + sd
            + "The Fresnel "
            + 'scale at Ka-band is sqrt(55/209) times FRESNEL SCALE."'
        ),
        (
            '"The radius of a sphere centered at'
            + sd
            + "Saturn and is tangent to the line-of-sight from "
            + "the spacecraft (at"
            + sd
            + "the SPACECRAFT EVENT TIME) "
            + "to Earth (at the OBSERVED EVENT TIME). It"
            + sd
            + "identified the minimum radius of hypothetical "
            + "spherically symmetric"
            + sd
            + "atmosphere that is "
            + "sensed by radio signal along its path in the"
            + sd
            + 'absence of refractive ray bending."'
        ),
        (
            '"X component of the spacecraft position'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
            + "frame (ux, uy, uz). "
            + "The basis vector ux is parallel to the"
            + sd
            + "projection of the line-of-sight from the spacecraft "
            + "(at the"
            + sd
            + "SPACECRAFT EVENT TIME) to Earth "
            + "(at the OBSERVED EVENT TIME)"
            + sd
            + "on the ring plane. "
            + "The basis vector uz is in the"
            + sd
            + "direction of Saturn"
            "s pole. The basis vector uy "
            + "is defined by the"
            + sd
            + 'right-hand rule."'
        ),
        (
            '"Y component of the spacecraft position'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
            + "frame (ux, uy, uz). "
            + "See description of planetocentric reference"
            + sd
            + "frame (ux, uy, uz) in the description of column "
            + 'SPACECRAFT POSITION X."'
        ),
        (
            '"Z component of the spacecraft position'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
        ...
        (
            '"X component of the spacecraft velocity'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
        ...
        (
            '"Y component of the spacecraft velocity'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
        ...
        (
            '"Z component of the spacecraft velocity'
            + sd
            + "vector (at the SPACECRAFT EVENT TIME) in a "
            + "planetocentric reference"
            + sd
        ...
        (
            '"The latitude of the position vector from'
            + sd
            + "the observing DSN station to the spacecraft computed at the "
            + "OBSERVED"
            + sd
            + "EVENT TIME. The position vector is "
            + "computed in the topographic reference"
            + sd
