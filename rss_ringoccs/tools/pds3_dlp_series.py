...
def write_dlp_series_data(dlp_inst, out_file):
    ...
    #   as an attribute
    tau_norm_vals = -np.sin(abs(dlp_inst.B_rad_vals)) * np.log(dlp_inst.p_norm_vals)
    ...
    if hasattr(dlp_inst, "ul_rho_km_vals"):
        format_str = (
            "%14.6F,"
            + "%10.6F,"
            + "%10.6F,"
        ...
        for n in range(npts):
            f.write(
                ...
                % (
                    ...
                    dlp_inst.t_ul_spm_vals[n],
                    dlp_inst.t_ul_ret_spm_vals[n],
                    dlp_inst.ul_rho_km_vals[n],
                    dlp_inst.ul_phi_rl_deg_vals[n],
                    dlp_inst.ul_phi_ora_deg_vals[n],
                    "\r\n",
                )
            )
    else:
...
def get_dlp_series_info(rev_info, dlp_inst, series_name, prof_dir):
    ...
    MINIMUM_RING_RADIUS = str(round(min(dlp_inst.rho_km_vals), 4)) + "   <km>"
    MAXIMUM_RING_RADIUS = str(round(max(dlp_inst.rho_km_vals), 4)) + "   <km>"
    MINIMUM_RING_LONGITUDE = (
        str(round(min(np.degrees(dlp_inst.phi_rl_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_RING_LONGITUDE = (
        str(round(max(np.degrees(dlp_inst.phi_rl_rad_vals)), 4)) + "   <deg>"
    )
    MINIMUM_OBSERVED_RING_AZIMUTH = (
        str(round(min(np.degrees(dlp_inst.phi_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_OBSERVED_RING_AZIMUTH = (
        str(round(max(np.degrees(dlp_inst.phi_rad_vals)), 4)) + "   <deg>"
    )
    MINIMUM_OBSERVED_RING_ELEVATION = (
        str(round(min(np.degrees(dlp_inst.B_rad_vals)), 4)) + "   <deg>"
    )
    MAXIMUM_OBSERVED_RING_ELEVATION = (
        str(round(max(np.degrees(dlp_inst.B_rad_vals)), 4)) + "   <deg>"
    )
    ...
    FILE_DESCRIPTION = (
        ...
        + "of Saturn"
        "s rings, that is, "
        + "calibrated profiles before reconstruction"
        + sd
        + "to remove diffraction effects. The frequency/phase "
        + "measurements"
        + sd
        + "reference is the Cassini UltraStable "
        + "Oscillator (USO). The diffraction"
        + sd
        + "reconstruction "
        + "is carried out using algorithms described at length in"
        + sd
        ...
        + "RING_OBSERVATION_ID"
        + sd
        + "(listed in the header of the LBL "
        + "file) as this product. These include"
        + sd
        + "one or more "
        + "reconstructed (TAU) files for the same occultation but"
        + sd
        + "at different resolutions, and two which provide geometry and "
        + "calibration"
        + sd
        + "data. The latter two have file names "
        + "constructed from the same root as"
        + sd
        ...
        + "DLP profiles. It"
        "s the inverse "
        + "of the highest spatial frequency preserved"
        + sd
        + "in the data, which is determined by the bandwidth of "
        + "the lowpass filter"
        + sd
        + "used to decimate the calibrated "
        + "data. The archived DLP files have a shortest"
        + sd
        + "resolvable wavelength of 0.5 km (the "
        "500M"
        " descriptor "
        + "in the file name),"
        + sd
        + "and are sampled every 0.25 km "
        + "(sampled at the Nyquist rate). They are"
        + sd
        + "provided for the benefit of users wanting to experiment with "
        + "their own"
        + sd
        + "diffraction reconstruction implementation. "
        + "Recovered profiles should look"
        + sd
        + "identical to the "
        + "archived 1 km resolution reconstructed profiles provided"
        + sd
        ...
        + sd
        + "0.75 km, where "
        + "the latter is defined by Eq. 19 of MAROUFETAL1986."
        + sd
        + " "
        + sd
        ...
        + "at X-band, its value near the beginning"
        + sd
        + "of the "
        + "Cassini orbital tour. The frequency is coherently "
        + "scaled by 3/11"
        + sd
        + "for S-band and by 209/55 for Ka-band. "
        + "The exact USO frequency changed"
        + sd
        + "slightly (at the Hz level) during the USO lifetime. "
        + "The change negligibly "
        + sd
        + "impacts the archived products. The same holds "
        + "true for the Allan deviation"
        + sd
        + "characterizing the "
        + "stability of the USO. Typical values of the Cassini USO"
        + sd
        + "Allan deviation is 2E-13 over 1 s and 1E-13 over 10-100 s. "
        + "The USO Allan"
        + sd
        + "deviation changed little over the "
        + "USO lifetime."
        + sd
        + " "
        + sd
    ...
    object_descriptions = [
        ...
        (
            '"Power (amplitude square) of the measured'
            + sd
            + "ring-attenuated diffraction-limited radio signal, "
            + "normalized by its"
            + sd
            + "value in the absence of the "
            + "rings (normalized to unity in free-space)."
            + sd
            + "The value may be used to compute the measured "
            + "diffraction-limited"
            + sd
            + "oblique optical depth "
            + "as the negative natural logarithm of the NORMALIZED"
            + sd
            + 'SIGNAL POWER."'
        ),
        (
            '"The normal optical depth obtained'
            + sd
            + "from its measured "
            + "oblique value csaled by the sine of the absolute"
            + sd
