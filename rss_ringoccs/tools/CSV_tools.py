...
class ExtractCSVData:
    ...
    def __init__(self, geo, cal, dlp, tau=None, verbose=True, use_deprecate=False):
        ...
        error_check.check_is_real(
            self.rho_corr_timing_km_vals, "rho_corr_timing_km_vals", fname
        )
        error_check.check_is_real(
            self.raw_tau_threshold_vals, "raw_tau_threshold_vals", fname
        )
        error_check.check_is_real(
            self.rho_corr_pole_km_vals, "rho_corr_pole_km_vals", fname
        )
        error_check.check_is_real(self.rho_dot_kms_vals, "rho_dot_kms_vals", fname)
        ...
        error_check.check_two_pi(
            self.phase_rad_vals, "phase_rad_vals", fname, deg=False
        )
        ...
        error_check.check_lengths(
            self.phi_rad_vals, self.rho_km_vals, "phi_rad_vals", "rho_km_vals", fname
        )
        error_check.check_lengths(
            self.raw_tau_vals, self.rho_km_vals, "raw_tau_vals", "rho_km_vals", fname
        )
        error_check.check_lengths(
            self.B_rad_vals, self.rho_km_vals, "B_rad_vals", "rho_km_vals", fname
        )
        error_check.check_lengths(
            self.D_km_vals, geo_rho, "D_km_vals", "rho_km_vals", fname
        )
        error_check.check_lengths(
            self.rho_dot_kms_vals, geo_rho, "D_km_vals", "rho_km_vals", fname
        )
        ...
        self.f_sky_hz_vals = np.interp(self.t_oet_spm_vals, cal_spm, self.f_sky_hz_vals)
        self.D_km_vals = np.interp(self.rho_km_vals, geo_rho, self.D_km_vals)
        self.rho_dot_kms_vals = np.interp(
            self.rho_km_vals, geo_rho, self.rho_dot_kms_vals
        )
        ...
        if not isinstance(tau, type(None)):
            ...
            self.tau_vals = np.interp(
                self.tau_rho, tau_dat.rho_km_vals, tau_dat.raw_tau_vals
            )
            self.phase_vals = np.interp(
                self.tau_rho, tau_dat.rho_km_vals, np.deg2rad(tau_dat.phase_deg_vals)
            )
        ...
        input_vars = {"GEO Data": self.geo, "CAL Data": self.cal, "DLP Data": self.dlp}
...
class GetUranusData:
    def __init__(self, geodata, dlpdata, tau=None, verbose=False, occ="E"):
        ...
        error_check.check_lengths(
            self.D_km_vals, self.rho_km_vals, "D_km_vals", "rho_km_vals", fname
        )
        error_check.check_lengths(
            self.rho_dot_kms_vals, self.rho_km_vals, "D_km_vals", "rho_km_vals", fname
        )
        ...
        input_vars = {"GEO Data": geodata, "DLP Data": dlpdata}
        input_kwds = {"Use of Verbose": verbose}
        self.history = write_history_dict(input_vars, input_kwds, __file__)
