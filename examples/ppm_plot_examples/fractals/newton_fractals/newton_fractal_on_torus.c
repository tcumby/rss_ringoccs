...
const unsigned int size = 2*1024;
...
int main(void)
{
    ...
    for (y=0; y<size; ++y)
    {
        ...
        for (x=0; x<size; ++x)
        {
            ...
            P = rssringoccs_Inverse_Orthographic_Projection_Torus(planar_Z, r, R);
            Pz = rssringoccs_ThreeVector_Z(P);

            if (rssringoccs_Is_NaN(Pz))
            {
            ...
            else
            {
                ...
                rho = rssringoccs_Double_Sqrt(Px*Px + Py*Py) - R;

                theta = rssringoccs_Double_Arctan2(Py, Px);
                phi = rssringoccs_Double_Arctan2(Pz, rho);

                proj_P = rssringoccs_Stereographic_Projection_Torus(phi, theta, r, R);
                ...
                min = rssringoccs_CDouble_Dist(root, roots[0]);
                ind = 0;

                for (n=1; n<NRoots; ++n)
                {
                    temp = rssringoccs_CDouble_Dist(root, roots[n]);
                    if (temp < min) {
                        min = temp;
                        ind = n;
                    }
